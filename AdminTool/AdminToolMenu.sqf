...
if ((getPlayerUID player) in ["76561197978126241"]) then { // Admin PlayerID Eintragen <--- Hier Bitte Alle Admins eintragen!
	if ((getPlayerUID player) in ["76561197978126241"]) then { // Mod PlayerID Eintragen
        adminmenu =
        [
			["",true],
	...
	if ((getPlayerUID player) in ["76561197978126241"]) then { // Admin PlayerID Eintragen
        adminmenu =
        [
			["",true],
	...
	if ((getPlayerUID player) in ["76561197978126241"]) then { // Super Admin PlayerID Eintragen
		adminmenu =
		[
			["",true],
...
[
	["Flugzeuge",true],
		...
		["B_Plane_CAS_01_F", [2],  "", -5, [["expression", format[_EXECscript7,"B_Plane_CAS_01_F.sqf"]]], "1", "1"],
		["Exit", [13], "", -3, [["expression", ""]], "1", "1"]
];
PanzerMenu = 
[
