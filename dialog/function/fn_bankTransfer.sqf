...
if((lbCurSel 2703) == -1) exitWith {hint "Wähle einen Empfänger aus"};
...
if((_val + _tax) > life_atmcash) exitWith {hint format["Du hast nicht genug Geld auf deinem Konto, eine Überweisung von $%1 kostet $%2 an Gebühren.",_val,_tax]};
...
hint format["Du hast $%1 an %2 überwiesen.\n\nEine Gebühr von $%3 wurde automatisch erhoben.",[_val] call life_fnc_numberText,_unit getVariable["realname",name _unit],[_tax] call life_fnc_numberText];
