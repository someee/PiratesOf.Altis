...
//Shop Title Name
ctrlSetText[3103,"Polizei Bekleidungen"];
...
switch (_filter) do
{
	...
	case 0:
	{
		...
		if((__GETC__(life_coplevel) > 5) && license_cop_sek) then
		{
			_ret set[count _ret,["U_B_CombatUniform_mcam","SEK Uniform",550]];
		};
		if(__GETC__(life_coplevel) > 7) then
		{
			_ret set[count _ret,["U_C_Poor_1","Chef!",550]];
		};
	};
	...
	case 1:
	{
		...
		if(__GETC__(life_coplevel) > 1) then
		{
			_ret set[count _ret,["H_Beret_02","Chef!",120]];
		};
	};
